...
When small fish are absent, one approach to improve the estimates of the growth parameters is to constrain tzero using an informative penalty prior. Assuming a normal distribution for the prior, the above-described negative log-likelihood is modified to include the normal penalty, i.e.

$$
\lambda=\frac{n}{2}\bigg[\ln(2\pi)+2\ln(\hat{\sigma})+1\bigg] + \frac{1}{2}\ln(2\pi\sigma^2_p)+\frac{(t_0-\hat{t}_0)^2}{2\sigma^2_p}
$$
where $t_0$ is the value for the hypothetical age at zero length parameter being estimated, $\hat{t}_0$ is the mean for the the prior for this parameter (as inputted by the user) and $\sigma_p$ is the standard deviation for the prior (also inputted by the user). 
