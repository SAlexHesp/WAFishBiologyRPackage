...
Trends in mean monthly GSIs can be impacted by variations in the sizes of fish collected in monthly samples. Thus, some researchers prefer to consider trends in mean monthly gonad weights (or GSIs) after they have been standardised for length effects, such as by using Analyis of Covariance (ANCOVA) or a generalised linear model(glm) (e.g. Cossington et al., 2011). WAFishBiology calculates standardised mean monthly gonad weights (and associated 95% confidence limits), using the glm function within R. The model used to calculate mean gonad weights according to fish total length and month is

$$
GW_{i,k}=M_k+L_i{\sim} N(0,\sigma^2)
$$
